...
function extractPersonal(
  ...
  const website =
    allUrls.find((u) => !LINKEDIN_RE.test(u) && !GITHUB_RE.test(u)) ??
    undefined;
  ...
  const locationLine = headerLines.find((l) =>
    isLikelyLocationLine(l, nameLine, titleLine),
  );
...
function buildExperienceFromDraft(
  job: DraftExperience,
  idx: number,
): ResumeExperience | null {
  const bullets = job.bullets
    .map((bullet) => cleanFieldValue(bullet))
    .filter(Boolean);
  const fallbackDescription =
    bullets[0] ??
    job.notes.map((note) => cleanFieldValue(note)).find(Boolean) ??
    "";

  if (!job.title && !job.company && !job.period && !fallbackDescription)
    return null;
...
function isLikelyPeriodLine(line: string): boolean {
  ...
  if (normalized === "perioada" || normalized.startsWith("perioada "))
    return true;
...
function looksLikeStandaloneJobTitle(line: string): boolean {
  ...
  if (EMAIL_RE.test(line) || PHONE_RE.test(line) || DATE_RE.test(line))
    return false;
...
function extractTrailingPeriod(
  line: string,
): { period: string; remainder: string } | null {
  const match = line.match(TRAILING_DATE_RANGE_RE);
  const period = cleanFieldValue(match?.[1] ?? "");
  if (!period) return null;

  const remainder = cleanFieldValue(
    line.replace(TRAILING_DATE_RANGE_RE, "").replace(/[()]+$/g, ""),
  );
...
function appendToLastBullet(current: DraftExperience, line: string): boolean {
  ...
  current.bullets[lastIdx] = `${current.bullets[lastIdx]} ${continuation}`
    .replace(/\s+/g, " ")
    .trim();
  return true;
}
...
function parseExperienceSection(lines: string[]): ResumeExperience[] {
  ...
  for (const rawLine of lines) {
    ...
    if (titleValue !== null) {
      if (titleValue) {
        if (current.title && hasJobContent(current) && current.period)
          pushCurrent();
    ...
    const responsibilityValue = extractTaggedValue(
      line,
      RESPONSIBILITY_LINE_RE,
    );
...
const EDU_PERIOD_PREFIX_RE =
  /^(?:perioada|period(?:ul)?|period|dates?)\s*[:\-–—]?\s*(.*)$/i;
const EDU_PERIOD_SUFFIX_RE =
  /^(.*?)\s+(?:perioada|period(?:ul)?|period|dates?)\s*$/i;
...
function formatEducationEntry(entry: EducationDraft): string {
  const heading = cleanFieldValue(
    entry.qualification || entry.institution || "Education",
  );
  ...
  if (
    entry.institution &&
    normalizeForMatch(entry.institution) !== normalizeForMatch(heading)
  ) {
...
function formatEducationSectionContent(lines: string[]): string {
  ...
  for (const rawLine of lines) {
    ...
    const periodValue = extractPrefixOrSuffixValue(
      line,
      EDU_PERIOD_PREFIX_RE,
      EDU_PERIOD_SUFFIX_RE,
    );
    if (periodValue !== null) {
      if (
    ...
    const fieldValue = extractPrefixOrSuffixValue(
      line,
      EDU_FIELD_PREFIX_RE,
      EDU_FIELD_SUFFIX_RE,
    );
    ...
    if (
      ...
      (looksLikeCompanyLine(line) ||
        /(universit|academy|facult|liceu|school|college|institut)/i.test(line))
    ) {
...
function buildDynamicSection(
  ...
  const baseType = section.type.startsWith("custom:")
    ? "leadership"
    : section.type;
  const type = (SECTION_TYPE_MAP[baseType] ??
    "leadership") as ResumeDynamicSection["type"];
