...
- [ ] post-monetizare: unify API error-response shapes (2026-09-02/03 audit: `parse` and `cover-letter/pdf`'s raw 429 `NextResponse` branches were already folded into `jsonWithRequestId`; `apiError`/`apiSuccess` is still the live convention in 10 files — account/delete, account/export, resumes, resumes/[id], cover-letters, cover-letters/[id], ai-reviews, ai-reviews/[id], rate-limit-status, sentry-example-api. Still a real, ~10-file refactor — not done.)
- [DONE] 2026-09-02/03: Zod validation for hand-parsed query params — admin/rate-limit GET (`adminRateLimitGetSchema`) and cron `parseCronOptions` (`cronOptionsQuerySchema`) done. `auto-fix`'s `dryRun` reviewed and left as `=== 'true'`: it's a single boolean with a safe default on any malformed input, a Zod schema would be ceremony not correctness.
- [DONE] 2026-09-02/03: pdfjs-dist stays — `Dockerfile:44`'s `standard_fonts` COPY confirms it's still live at runtime for `src/lib/pdf-text-extract.ts` (anonymous free-ATS-checker text extraction, deliberately not routed through the paid resume-parser service). Bumped 5.x -> 6.x (patched GHSA-hq66-cqwq-w95j), migrated the `isEvalSupported`/`destroy()` breaking API changes, added its first test coverage (`tests/lib/pdf-text-extract.test.ts`).
